...
function renderTripsSB(){{
  ...
  DATA.trips.forEach((t,i)=>{{
    ...
    groups[k].push({{...t,_i:i}});
  }});
  let h='';
  for(const [mo,trips] of Object.entries(groups)){{
    h+=`<div class="m-label">${{mo}}</div>`;
    trips.forEach(t=>{{
      const k=tripKey(t), isOn=selTrip&&tripKey(selTrip)===k;
      const ta=a.trips[k]||{{}};
      const hasNote=ta.journal||(ta.lessons&&ta.lessons.length);
      h+=`<div class="t-item ${{isOn?'on':''}}" onclick="selT(${{t._i}})">
        <div class="t-dot"></div>
        <div class="t-info">
          <div class="t-city">${{flag(t.country)}} ${{esc(t.city)}}</div>
...
function renderDTab(tab,ta,people,t){{
  ...
  <div class="card">
    ...
    <textarea id="j-input" rows="12" placeholder="What happened? How did it feel? What surprised you…">${{esc(ta.journal||'')}}</textarea>
    <div style="margin-top:10px;display:flex;align-items:center">
      <button class="btn btn-p" onclick="saveJournal()">Save</button>
      <span class="flash" id="flash">✓ Saved to file</span>
    </div>
  </div>`;
...
function saveJournal(){{
  ...
  a.trips[k]=a.trips[k]||{{}};
  a.trips[k].journal=document.getElementById('j-input').value;
  persist(a);
...
// ── Boot ──────────────────────────────────────────────────────────────────────
initNotes().then(()=>{{ initMap(); renderSB(); }});
</script>
</body>
</html>"""
